// import WxworksuiteTest from './WxworksuiteTest'
// import WxworksuiteTest2 from './WxworksuiteTest/index2'
import WxworksuiteLoadding from './WxworksuiteLoadding'
import WxworksuiteBaseOpendata from './WxworksuiteOpendata/base'
import WxworksuiteOpendata from './WxworksuiteOpendata'
import WxworksuiteTree from './WxworksuiteTree/tree'
import WxworksuiteTreenode from './WxworksuiteTree/treenode'
import WxworksuiteOrganizationpickMobile from './WxworksuiteOrganizationpick/mobile'
import WxworksuiteOrganizationpickWeb from './WxworksuiteOrganizationpick/web'
import WxworksuiteMemberpickMobile from './WxworksuiteMemberpick/mobile'
import WxworksuiteMemberpickWeb from './WxworksuiteMemberpick/web'

const components: any = {
  // WxworksuiteTest,
  // WxworksuiteTest2,
  WxworksuiteLoadding,
  WxworksuiteBaseOpendata,
  WxworksuiteOpendata,
  WxworksuiteTree,
  WxworksuiteTreenode,
  WxworksuiteOrganizationpickMobile,
  WxworksuiteOrganizationpickWeb,
  WxworksuiteMemberpickMobile,
  WxworksuiteMemberpickWeb,
}

export default components
